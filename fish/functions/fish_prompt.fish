function fish_prompt -d 'Write out the prompt'
    ...
    set -l prompt_date (printf "%s[%s]%s" (set_color yellow) (gdate +'%F %T') (set_color normal))
    ...
    if test -n "$is_git_repository"
        ...
        if test -n "$current_branch"
          ...
          # emojiと']'が重ならないように半角スペースを置いておく
          set prompt_git (printf "%s[%s ]%s" (set_color cyan) $current_branch (set_color normal))
        end
    end

    printf "%s%s%s\n%s " $prompt_date $prompt_user_pwd $prompt_git $prompt_symbol
end
